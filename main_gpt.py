...
def chatbot_agent(user_message):
    ...
    system_prompt = (
        "You are a capable virtual assistant with memory. You provide information about different topics to the user, and to be inquisitive. "
        "You must provide a human-readable response to the user, as if it is a normal conversation. "
        "Your job is to execute commands in the terminal based on the user's requests. "
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if user_input.lower() in ["exit", "quit"]:
            print("Goodbye!")
            conversation_history.clear()  # Reset chat history at the end of session
            break
        final_text, convo_ended = chatbot_agent(user_input)
        print("Bot:", final_text)

        # If GPT-4o generated a command, execute it
        if "ssh" in final_text or "cd " in final_text or "ls" in final_text or "rm " in final_text:
            from execute_command import write_and_execute_command
            write_and_execute_command(final_text)
